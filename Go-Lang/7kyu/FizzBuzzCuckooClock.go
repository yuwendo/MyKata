...
Have fun!
*/
func fizzBuzzCuckooClock(timeStr string) string {
	// your code here
	tempTime, err := time.Parse("15:04", timeStr)
	if err != nil {
		return ""
	}
	...
	if tempTime.Minute() == 0 {
		times := tempTime.Hour() % 12
		if times == 0 {
			times = 12
		}
		for i := 0; i < times; i++ {
			result = append(result, "Cuckoo")
		}
		return strings.Join(result, " ")
	} else if tempTime.Minute() == 30 {
		return "Cuckoo"
	}

	if tempTime.Minute()%3 == 0 {
		result = append(result, "Fizz")
	}

	if tempTime.Minute()%5 == 0 {
